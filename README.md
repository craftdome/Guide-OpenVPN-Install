...
1. В файле `/etc/iptables/rules.v4` интерфейс `ens32` меняем на `tun1`.
```
...
3. Создаём `/etc/openvpn/upstream-route.sh`.
...
6. Проверяем наличие интерфейса **tun1**. Если интерфейс присутствует, то всё сделано правильно, можно проверять подключение.
```
ip addr show tun1
```
